...
While the server is unavailable, the bootstrap page displays a localized startup notice instead of a blank source and retries every three seconds. As soon as the health check succeeds, it navigates the browser source to the widget as its top-level document. This preserves OBS transparency and avoids the opaque Chromium surface that can occur when an HTTP widget is embedded in a local-file iframe. If the server stops later, the loaded widget displays the localized connection notice and reconnects without resetting the OBS source.
